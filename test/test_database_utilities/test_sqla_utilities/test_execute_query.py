...
class TestWithPopulatedSQLServer:
    ...
    def execute_query_mssql_setup_and_teardown(
        self,
        ahjo_config,
        mssql_sample,
        mssql_engine,
    ...
    def test_execute_query_should_return_rows(self):
        result = ahjo.execute_query(self.engine, query="SELECT * FROM store.Clients")
        assert result
    ...
    def test_execute_query_should_not_return_headers(self):
        result = ahjo.execute_query(self.engine, query="SELECT name FROM store.Clients")
        assert result[0][0] != "name"

    def test_execute_query_should_return_headers(self):
        result = ahjo.execute_query(
            self.engine, query="SELECT name FROM store.Clients", include_headers=True
        )
        assert result[0][0] == "name"

    def test_execute_query_should_return_rows_from_proc(self):
        result = ahjo.execute_query(self.engine, query="EXEC store.ReturnClients")
        assert result

    def test_execute_query_should_not_return_rows_from_proc(self):
        result = ahjo.execute_query(self.engine, query="EXEC store.UpdateClients")
        assert result == []
