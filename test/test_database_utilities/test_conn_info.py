...
@pytest.mark.parametrize("hostname, port", [("localhost", 1433), ("localhost", "5432"), ("my-host-name", 34)])
...
@pytest.mark.parametrize("hostname, port", [("localhost", None), ("localhost", 0), ("my-host-name", None)])
...
def test_conn_info_server_should_be_created_by_ahjo(read_config):
    conn_info = ahjo.create_conn_info(read_config)
    server_string = ahjo._create_server_string(read_config.get('target_server_hostname'), read_config.get('sql_port'))
