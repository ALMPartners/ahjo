...
def prepared_sample(tmpdir_factory, pytestconfig):
    def prepare_sample_for_tests(
        sample_name, host_param, port_param, usrn_param, pass_param
    ):
...
def run_alembic_action():
    ...
    def execute_alembic(action, target):
        ...
        alembic_config.cmd_opts = Namespace(x=["main_config=config_development.json"])
        if action == "upgrade":
...
def populate_table():
    ...
    def insert_to_table(engine, table_name):
        ...
        if len(splitted) > 1:
            ...
            target_table = Table(
                table_name, MetaData(), schema=table_schema, autoload_with=engine
            )
        else:
