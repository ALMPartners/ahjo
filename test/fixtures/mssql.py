...
def mssql_master_engine(request, ahjo_config, mssql_sample):
    ...
    if driver.lower() == "odbc driver 18 for sql server":
        query["TrustServerCertificate"] = config["sqla_url_query_map"][
...
def drop_mssql_objects():
    ...
    def drop_objects(engine):
        w_objects = [
            f"VIEW [{f.split('.')[0]}].[{f.split('.')[1]}]" for f in listdir(VIEWS_DIR)
        ]
        p_objects = [
            f"PROCEDURE [{f.split('.')[0]}].[{f.split('.')[1]}]"
            for f in listdir(PROC_DIR)
        ]
        f_objects = [
            f"FUNCTION [{f.split('.')[0]}].[{f.split('.')[1]}]"
        ...
        for db_object in database_objects:
            with engine.connect() as connection:
                ...
                with connection.begin():
                    connection.execute(
                        text(
                            f"BEGIN TRY DROP {db_object} END TRY BEGIN CATCH END CATCH"
                        )
                    )
