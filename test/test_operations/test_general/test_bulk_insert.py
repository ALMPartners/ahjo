...
class TestWithSQLServer:
    ...
    def bulk_insert_mssql_setup_and_teardown(
        ...
        with self.engine.connect() as connection:
            ...
            with connection.begin():
                connection.execute(
                    text(
                        f"DROP TABLE IF EXISTS {BULK_INSERT_SCHEMA}.{BULK_INSERT_TABLE}"
                    )
                )
    ...
    def test_bulk_insert_should_add_data_to_db(self):
        bulk_insert_into_database(
            self.engine, self.reflected_bulk_insert_table(), BULK_DATA
        )
        with self.engine.connect() as connection:
            result = connection.execute(
                text(
                    f"SELECT {COL_1_NAME}, {COL_2_NAME} FROM {BULK_INSERT_SCHEMA}.{BULK_INSERT_TABLE}"
                )
            ).fetchall()
    ...
    def test_bulk_insert_should_restore_default_values(self):
        ...
        bulk_insert_into_database(
            self.engine, self.reflected_bulk_insert_table(), BULK_SMALL_DATA
        )
        assert self.engine.dialect.use_insertmanyvalues == True
        assert self.engine.dialect.bind_typing == interfaces.BindTyping.SETINPUTSIZES
