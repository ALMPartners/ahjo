...
class TestAhjoUpgrade:
    ...
    def test_get_next_version_upgrade_without_next_upgrade_in_config(self):
        with pytest.raises(
            ValueError,
            match=r"The current database version \(a\) has no upgradable version in the upgrade actions. The next upgradable version is: b",
        ):
    ...
    def test_get_upgrade_version_path_with_missing_version(self):
        ...
        with pytest.raises(
            ValueError,
            match=r"Upgrade actions are not defined for the following versions: d.\nCheck that the upgrade actions are defined correctly.",
        ):
    ...
    def test_get_upgrade_version_path_with_multiple_heads(self):
        ...
        with pytest.raises(
            ValueError,
            match=r"Multiple latest versions found in the upgrade actions: d2, d1. Check that the upgrade actions are defined correctly.",
        ):
            self.ahjo_upgrade.get_upgrade_version_path(
                tag_graph=tag_graph,
    ...
    def test_validate_version_should_return_only_given_versions_actions(self):
        version_actions = self.ahjo_upgrade.validate_version(
            "v3.1.3", {"v3.1.3": ["test-action"], "v3.1.4": ["test-action"]}, "v3.1.2"
        )
    ...
    def test_validate_upgrade_actions_with_valid_input(self):
        assert (
            self.ahjo_upgrade.validate_upgrade_actions(upgrade_actions=UPGRADE_ACTIONS)
            == True
        )
    ...
    def test_validate_upgrade_actions_with_invalid_type_int(self):
        with pytest.raises(
            ValueError,
            match="Upgrade actions for version v3.1.3 are not defined as list.",
        ):
    ...
    def test_validate_upgrade_actions_with_empty_list(self):
        with pytest.raises(
            ValueError, match="Upgrade actions are not defined for version v3.1.3."
        ):
            self.ahjo_upgrade.validate_upgrade_actions({"v3.1.3": []})

    def test_validate_upgrade_action_with_invalid_type_int(self):
        with pytest.raises(
            ValueError, match="Upgrade action is not defined as string or list."
        ):
            self.ahjo_upgrade.validate_upgrade_actions({"v3.1.3": [1]})

    def test_validate_upgrade_action_with_invalid_type(self):
        with pytest.raises(
            ValueError, match="Upgrade action name is not defined as string."
        ):
            self.ahjo_upgrade.validate_upgrade_actions({"v3.1.3": [[1]]})

    def test_validate_upgrade_action_with_valid_input(self):
        assert (
            self.ahjo_upgrade.validate_upgrade_actions({"v3.1.3": ["test-action"]})
            == True
        )

    def test_validate_upgrade_action_with_invalid_action_parameter_type(self):
        with pytest.raises(
            ValueError, match="Upgrade action parameters are not defined as dictionary."
        ):
