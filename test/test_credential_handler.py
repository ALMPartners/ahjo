...
def execute_get_credentials_with_varying_input(tmp_path, monkeypatch):
    ...
    def get_credentials(
        ...
        if usrn_file_name and usrn_file_content is not None:
            ...
            usrn_file_path.write_text(f"cred={usrn_file_content}", encoding="utf-8")
        ...
        if pw_file_name and pw_file_content is not None:
            ...
            pw_file_path.write_text(f"cred={pw_file_content}", encoding="utf-8")
        ...
        if ask_pw:
            return ahjo.get_credentials(
                usrn_file_path=usrn_file_path, pw_file_path=pw_file_path
            )
        else:
            return ahjo.get_credentials(
                usrn_file_path=usrn_file_path, pw_file_path=pw_file_path, pw_prompt=None
            )
...
def test_credentials_should_be_read_from_file_when_both_paths_given(
    execute_get_credentials_with_varying_input,
):
...
def test_credentials_should_be_asked_when_password_file_not_given(
    execute_get_credentials_with_varying_input,
):
...
def test_credentials_should_be_asked_when_both_files_not_given(
    execute_get_credentials_with_varying_input,
):
...
def test_windows_authentication_from_files_should_return_empty_strings(
    execute_get_credentials_with_varying_input,
):
...
def test_windows_authentication_from_input_should_return_empty_strings(
    execute_get_credentials_with_varying_input,
):
...
def test_input_should_be_asked_when_file_missing(
    execute_get_credentials_with_varying_input,
):
...
def test_password_should_not_be_asked_if_pw_prompt_is_none(
    execute_get_credentials_with_varying_input,
):
