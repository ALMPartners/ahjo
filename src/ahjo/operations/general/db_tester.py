...
class DatabaseTester:
    ...
    def __init__(
        ...
        table: Table = None,
        save_test_results_to_db: bool = False,
    ):
        ...
        for filepath, output in file_results.items():
            ...
            for row in output:
                ...
                for col_i, col in enumerate(row):
                    ...
                    if (
                        result_col_exists
                        and col_i == output_columns.index(result_col_name)
                ...
                if isinstance(col_ordering, list) and all(
                    col in output_columns for col in col_ordering
                ):
                    new_row = []
                    for col in col_ordering:
                        try:
                            ...
                            logger.error(
                                f"Column '{col}' not found in the output columns: {output_columns}"
                            )
                            raise e
    ...
    def save_results_to_db(self, file_results: dict):
        ...
        try:
            ...
            connection = (
                connectable.connect() if type(connectable) == Engine else connectable
            )
            ...
            for filepath, output in file_results.items():
                ...
                if "batch_id" in test_table_columns:
                    batch_id = connection.execute(
                        text(f"SELECT MAX(batch_id) + 1 FROM {self.table.fullname}")
                    ).scalar()
                ...
                if len(columns) == 0:
                    raise ValueError(
                        f"No matching columns between the test output and the test table columns. Test output columns: {output_columns}, table columns: {test_table_columns}"
                    )
                ...
                for row in output[1:]:
                    ...
                    if (
                        "batch_id" in test_table_columns
                        and "batch_id" not in output_columns
                    ):
                        row_dict["batch_id"] = batch_id

                    # Add the test file name to the row
                    if (
                        "test_file" in test_table_columns
                        and "test_file" not in output_columns
                    ):
