...
def create_conn_info(conf: dict) -> dict:
    ...
    if isinstance(driver, str):
        ...
        if driver_lower.startswith(
            "odbc driver"
        ):  # ODBC specific default connection parameters

            if "Encrypt" not in sqla_url_query_map:
                sqla_url_query_map["Encrypt"] = (
    ...
    if azure_auth_lower == "azureidentity":
        ...
        token_url = (
            azure_identity_settings.get("token_url")
            if isinstance(azure_identity_settings, dict)
            and "token_url" in azure_identity_settings
            else "https://database.windows.net/.default"
        )
        ...
        try:
            ...
            graph_token = azure_credentials.get_token(
